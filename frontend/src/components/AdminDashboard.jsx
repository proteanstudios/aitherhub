    ...
    <div className="w-full max-w-5xl mx-auto px-4 py-8">
      ...
      <section className="mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard
            label="総動画数"
            value={data_volume.total_videos}
            unit="本"
            color="orange"
          />
          <StatCard
            label="解析済"
            value={data_volume.analyzed_videos}
            unit="本"
            color="green"
          />
          <StatCard
            label="解析待ち"
            value={data_volume.pending_videos}
            unit="本"
            color="yellow"
          />
          <StatCard
            label="総動画時間"
            value={data_volume.total_duration_display}
            color="blue"
          />
        </div>
      </section>
      ...
      <section className="mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <StatCard
            label="画面収録数"
            value={video_types.screen_recording_count}
            unit="本"
            color="purple"
          />
          <StatCard
            label="クリーン動画数"
            value={video_types.clean_video_count}
            unit="本"
            color="indigo"
          />
          <StatCard
            label="最新アップ日"
            value={formatDate(video_types.latest_upload)}
            color="gray"
            small
          />
        </div>
      ...
      <section className="mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <StatCard
            label="総ユーザー"
            value={user_scale.total_users}
            unit="人"
            color="orange"
          />
          <StatCard
            label="配信者数"
            value={user_scale.total_streamers}
            unit="人"
            color="red"
          />
          <StatCard
            label="今月アップ人数"
            value={user_scale.this_month_uploaders}
            unit="人"
            color="teal"
          />
        </div>
...
function StatCard({ label, value, unit, color = "gray", small = false }) {
  ...
  return (
    <div
      className={`rounded-xl border p-4 ${colorMap[color] || colorMap.gray} transition-all duration-200 hover:shadow-md`}
    >
